---
mode: agent
---
Use ".github/instructions/enterprise-development-template.instructions.md" and ".github/instructions/launcher.instructions.md".

PROJECT=<NAME>
DESCRIPTION=<brief-project-description>
TECH_STACK=<preferred-technologies>
TIMELINE=<project-timeline>
TEAM_SIZE=<team-size>

Mission: Initialize new projects using enterprise development template with 4-phase workflow.

Do now:
- Apply enterprise development template framework
- Initialize project scaffolding with modern tech stack
- Set up comprehensive CI/CD pipeline
- Configure testing infrastructure (unit/integration/E2E)
- Create development environment setup
- Generate project documentation templates
- Store project plan in MemoraiMCP for continuation

Deliver:
- Complete project scaffolding with folder structure
- CI/CD pipeline configuration files
- Testing framework setup (Jest/Vitest + Playwright)
- Development environment configuration
- README.md with setup instructions
- Project plan stored in MemoraiMCP
- Update /.agent-state/state.json with project initialization
