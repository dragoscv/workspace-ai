...
- **✅ MEMORAI MCP READY**: The production-grade MemoraiMCP system is fully operational and optimized
- **ALWAYS CHECK MEMORY FIRST**: Before starting any task, use `mcp_memoraimcp_recall` to check for existing plans, prompts, and task history
- **STORE IMPORTANT CONTEXT**: Store user plans, task lists, and multi-step procedures in memory with `mcp_memoraimcp_remember` and `metadata: {entityType: 'plan'}` or `{entityType: 'task_list'}`
...
- **PRODUCTION PERFORMANCE**: MemoraiMCP v@latest delivers 95% efficiency improvements and sub-3-second response times
