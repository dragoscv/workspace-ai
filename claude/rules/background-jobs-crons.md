...
## Correctness
- Every job must be IDEMPOTENT — it will be retried, double-triggered, and re-run after a partial failure. Guard with a processed-marker, unique constraint, or upsert; never assume exactly-once
...
- Make jobs resumable/batched — process in chunks with a cursor so a timeout doesn't lose all work
...
## Safety & gating
- Cron/webhook endpoints must authenticate (shared secret header / `Authorization: Bearer $CRON_SECRET` / OIDC) — never an unauthenticated public route that mutates data
...
- Never send real emails/SMS/invoices from dev or staging — route to a sink or hard-guard on env
...
## Verifying
- After deploying/changing a job, trigger it manually once and read the logs — don't wait for the schedule to prove it works
