...
- `curl` is an alias for Invoke-WebRequest in PS 5.1 â€” use `curl.exe` for real curl flags
- Quote every path; parentheses and spaces in Windows paths break unquoted commands
- Never inline-escape JSON into CLI args â€” write a temp file and pass it (`--flags-file`, `@file`, or `-InFile`)
...
- Non-BMP chars (emoji) must never be substring-sliced â€” lone UTF-16 surrogates break JSON APIs downstream
...
- `gcloud ... --format=json | ConvertFrom-Json`; never parse table output; a timed-out `gcloud create` may still have created the resource â€” LIST before retrying
