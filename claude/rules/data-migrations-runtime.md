...
## Generating
- Always `pnpm db:generate` �?" never hand-write migration SQL files unless doing a data backfill the generator can't express
- Immediately after generating, verify the new file is REGISTERED in `drizzle/meta/_journal.json` �?" an orphaned .sql file will never run in prod (this has happened: the monorepo 0252)
...
- In a shared clone, another agent may generate a migration concurrently �?' numbering collision. Re-check `drizzle/` right before committing; regenerate if your number is taken

## Applying
- Local: `pnpm db:push` for dev iteration; `pnpm db:migrate:local` (backup + migrate) when testing the real migration path
- Staging/prod: ONLY via the backup-and-confirm scripts (`db:migrate:staging`, `db:migrate:prod`) �?" never raw `drizzle-kit push` against prod
- Never run migrations against prod without a fresh backup verified to exist
- `db:push` against a large DB can hang minutes on introspection �?" that's normal; don't kill-and-retry blindly
...
- Destructive ops (DROP, TRUNCATE, type narrowing) require explicit user confirmation �?" always
...
## Drift
- If `db:generate` produces an unexpected diff you didn't cause, STOP �?" likely another agent's uncommitted schema change or prod drift. Investigate before committing
