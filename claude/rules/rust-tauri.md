...
## Security â€” the part that is easy to get wrong
...
- Keep the CSP restrictive in `tauri.conf.json`. `"csp": null` disables it
  entirely â€” that is not an acceptable shortcut.
...
- `#[tauri::command]` functions should be thin: validate â†’ call a plain Rust
  function â†’ map the error. Business logic in a testable non-Tauri module.
- Return `Result<T, String>` (or a serializable error enum) â€” a panic in a
...
- `cargo clippy -- -D warnings` and `cargo fmt --check` in CI. Treat clippy
  warnings as errors.
- No `unwrap()` / `expect()` on anything that can fail at runtime â€” reserve them
...
- `cargo build --release` for anything you benchmark; debug builds are 10â€“100Ã—
  slower and will mislead you.
- Cross-compiling from Windows to macOS/Linux needs the target toolchain â€” this
  usually belongs in CI, not on the dev box.
- The frontend build must run before `tauri build`; a stale `dist/` ships
  silently.
- Bundle identifiers and signing config are per-platform â€” verify the artifact,
