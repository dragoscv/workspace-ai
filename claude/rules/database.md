...
- Money: never floats. Use `numeric`/`decimal` with explicit precision, or integer minor units (bani/cents) â€” be consistent per project
...
- Parameterized queries only â€” never string-concatenate user input (Drizzle does this for you; don't bypass with raw `sql` + interpolation)
...
- **Expand â†’ migrate â†’ contract** for breaking changes: add new nullable column â†’ backfill â†’ switch reads/writes â†’ drop old (across deploys, never in one)
...
- **Destructive ops (DROP, TRUNCATE, wide DELETE/UPDATE without WHERE) require explicit confirmation** and a verified backup â€” stop and ask
