...
backup-verification, chromatic, dr-drill…). CI is production infrastructure:
...
- Shard long E2E suites; run the full matrix nightly, a smoke subset on PRs
  (that is what `e2e-pr-smoke` vs `e2e-nightly` already do — keep the split).
...
- Least privilege: set `permissions:` explicitly per workflow, default to
  `contents: read`.
- Secrets via `secrets.*` only — never echoed, never in a step name, never in
  an artifact. Remember logs are public on public repos.
- `pull_request_target` runs with write scope against untrusted code — avoid it
...
- A retried-until-green test is not passing. Quarantine it, open an issue, fix
  the race — don't add `retries: 3` and move on.
...
- After deploying, the job must **verify the live state** (hit `/health`, read
  the live revision) — a green deploy step is not proof (see
...
- Branch protection lists the checks that actually matter. A required check
  that is `continue-on-error: true` is theatre — remove one or the other.
