...
- Lists use `<ul>/<ol>`; tabular data uses `<table>` with `<th scope>` — never divs faking tables
...
- Radix UI / shadcn primitives handle most of this — prefer them over hand-rolled widgets
...
- Never convey meaning by color alone — pair with icon, text, or pattern (critical for accounting status, invoice states)
...
- Lighthouse a11y score ≥ 95 on primary routes
