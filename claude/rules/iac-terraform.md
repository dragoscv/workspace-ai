...
resources cost money and deleting them can be unrecoverable â€” IaC changes get
the same scrutiny as a DB migration.

## The non-negotiable loop

1. `terraform plan -out=tfplan` â€” **read the plan**.
2. Confirm the counts: `N to add, M to change, K to destroy`.
3. **Any `destroy` or `replace` on a stateful resource stops the loop** â€”
   database, bucket, disk, KMS key. Report it and ask before applying.
4. `terraform apply tfplan` â€” apply the exact plan you read, not a fresh one.
...
- Remote backend (GCS), versioned and locked. Never commit `.tfstate` or
  `.tfstate.backup` â€” they contain resource attributes and often secrets.
...
- Drift means someone changed things in the console. Investigate **before**
  applying â€” a plan that "fixes" drift may be reverting a deliberate hotfix.
